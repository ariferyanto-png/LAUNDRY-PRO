...
function sendWhatsAppReceipt(id) {
  ...
  container.innerHTML = `
    <div class="report-card" style="margin-bottom: 16px;">
      ...
      <p><b>Kasir:</b> Arif</p>
      <p><b>Transaksi Masuk:</b> ${formatDate(item.date)}</p>
    ...
    ${(item.status !== "Antrian" && item.status !== "Batal") ? `
    <button type="button" class="submit-button" style="margin-bottom: 16px; background: #cbd5e1; color: var(--text); display: flex; align-items: center; justify-content: center; gap: 8px;" onclick="undoStatus(${item.id})">
      <i class="fas fa-undo"></i> Kembalikan Status
    </button>
    ` : ''}
...
function undoStatus(id) {
  ...
  const st = item.status ? item.status.toLowerCase().trim() : '';
  if (st === "selesai") item.status = "Siap Diambil";
  else if (st === "siap diambil" || st === "diambil") item.status = "Proses";
  else if (st === "proses" || st === "diproses") item.status = "Antrian";
  else return;

  saveData();
  renderAll();
  openTransactionDetail(id);
  showToast("Status dikembalikan ke: " + item.status);
}
function exportReportExcel() {
  let filtered = transactions.filter(item => item.status !== "Batal");
  ...
  csvContent += "No. Transaksi,Nama Pelanggan,Layanan,Total Harga,Status Pembayaran,Metode Pembayaran,Tanggal\n";

  filtered.forEach(item => {
    const itemsList = getTransactionItems(item);
    let layananStr = itemsList.map(it => `${it.serviceType} (${it.weight})`).join(" | ");
    let row = [
      `"TRX/${item.id}"`,
  ...
  link.setAttribute("download", "Laporan_Transaksi_Arsy_Laundry.csv");
